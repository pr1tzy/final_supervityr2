...
export function AIManager() {
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      let demo =
        'AceLink demo mode: full AI chat is not on the orchestrator backend yet. Use **Max AI** (sidebar) to submit leads to John, or open **AI Policies** / **Logs** for governance and pipeline activity.'
      if (lower.includes('lead') || lower.includes('intake')) {
        demo =
          'Open **Max AI** in the sidebar to POST a lead to John (`/api/orchestrator/webhook/lead`). Then watch **CRM** and **Logs** update.'
      } else if (lower.includes('policy') || lower.includes('policies')) {
        demo =
          'Policies live in Supabase `ai_policies`. John applies the **Minimum Lead Score** rule on intake (score &lt; 50 → workbench). Edit rules under **AI Policies**.'
      } else if (lower.includes('payment') || lower.includes('legal')) {
        demo =
          'Use **Legal Hub** and **Payments** to trigger phase 2/3 on a lead (e.g. Tristan). John calls Agents C and D on Supervity Auto.'
      }
      addMessage({ role: 'assistant', content: demo })
    } finally {
      setIsTyping(false)
    }
